...
def cutoff_cosine(distances, cutoff):
    ...
    return torch.where(distances <= cutoff,
...
def triu_index(num_species):
    ...
    species1, species2 = torch.combinations(species,
                                            r=2,
                                            with_replacement=True).unbind(-1)
...
# torch.jit.script
def compute_aev(species, coordinates, cell, shifts, triu_index, constants,
                sizes):
    ...
    atom_index1, atom_index2, shifts = neighbor_pairs(species == -1,
                                                      coordinates, cell,
                                                      shifts, cutoff)
    ...
    vec = coordinates.index_select(0, atom_index1) - coordinates.index_select(
        0, atom_index2) + shift_values
    ...
    # compute angular aev
    central_atom_index, pair_index1, pair_index2, sign1, sign2 = triple_by_molecule(
    ...
    species1_ = torch.where(sign1 == 1, species2[pair_index1],
                            species1[pair_index1])
    species2_ = torch.where(sign2 == 1, species2[pair_index2],
                            species1[pair_index2])
    ...
    index = central_atom_index * num_species_pairs + triu_index[species1_,
                                                                species2_]
...
class AEVComputer(torch.nn.Module):
    r"""Module that takes coordinates as input and outputs aevs.

    This NN module calculates the AEVs associated with a set of coordinates
    ...
    """
    __constants__ = [
        'Rcr', 'Rca', 'num_species', 'radial_sublength', 'radial_length',
    ...
    def __init__(self, Rcr, Rca, EtaR, ShfR, EtaA, Zeta, ShfA, ShfZ,
        ...
        self.angular_length = (
            self.num_species *
            (self.num_species + 1)) // 2 * self.angular_sublength
        ...
        default_cell = torch.eye(3,
                                 dtype=self.EtaR.dtype,
                                 device=self.EtaR.device)
        default_pbc = torch.zeros(3,
                                  dtype=torch.uint8,
                                  device=self.EtaR.device)
    ...
    def forward(self, input_):
        ...
        .. code-block:: python
            ...
            aev_computer = AEVComputer(*args)
            # call aev_computer.forward(input_)
            species, aev = aev_computer(input_)
        ...
        return species, compute_aev(species, coordinates,
                                    cell, shifts, self.triu_index,
                                    self.constants(), self.sizes)
