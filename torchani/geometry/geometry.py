...
def tile_into_tight_cell(species_coordinates, repeats=3, noise=None, delta=1.0, density=None):
    ...
    assert repeats >= 1, 'At least 1 molecule should be present'
    assert isinstance(repeats, int), 'Only integer repetitions are supported'
    ...
    x_disp = torch.arange(0, repeats, device=device) 
    displacements = torch.cartesian_prod(x_disp, x_disp, x_disp).double()
    num_displacements = len(displacements)
    ...
    if noise is not None:
        coordinates += torch.empty(coordinates.shape, device=device).uniform_(-noise, noise)
    box_length = box_length * repeats
    cell = torch.diag(torch.tensor([box_length, box_length, box_length], device=device, dtype=coordinates.dtype))
