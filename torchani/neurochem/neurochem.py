...
def _load_atomic_network_modules(species, dir_, dir_is_prefix=False, model_index=None):
    # Returns a ordered dict of networks from NeuroChem's network directory.
    # output can be used as the argument for ANIModel directly
...
def load_model(species, dir_):
    ...
    """
    modules = _load_atomic_network_modules(species, dir_)
    return ANIModel(modules)
