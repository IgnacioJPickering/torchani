...
try:
    ...
    path = os.getcwd()
training_path = os.path.join(path,"/home/ipickering/ani-datasets/ani1x_ani2x_dataset/ANI-1x_complete.h5")
...
# check the training RMSE to see overfitting (this is basically accuracy)
...
def init_weights(m):
    if type(m) == torch.nn.Linear:
        #best practice initialization, he = kaiming
        #is this the best for biases? dunno
        torch.nn.init.kaiming_normal_(m.weight)
        m.bias.data.fill_(0.01)
# some weight initialization technique is needed here probably
nn = torchani.ANIModel([atomic().apply(init_weights) for _ in range(4)])
print(nn)
...
# subtract energies by the self energies of all atoms for each molecule.
...
aniloss = torchani.ignite.MSELoss('energies')
anirmse = {'RMSE': torchani.ignite.RMSEMetric('energies')}

#############TRAINER AND EVALUATOR OBJECTS####################################
trainer = ignite.engine.create_supervised_trainer(container, optimizer, aniloss)
#I don't think it is necessary to do this
evaluator = ignite.engine.create_supervised_evaluator(container,metrics=anirmse)
##############################################################################
